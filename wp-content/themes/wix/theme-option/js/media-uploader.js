	...
	$(document).ready(function() {
		...
		function wix_add_file(event, selector) {
			...
			frame.on( 'select', function() {
				...
				selector.find('.upload-button').unbind().addClass('remove-file').removeClass('upload-button').val(wix_l10n.remove);
		...
		function wix_remove_file(selector) {
			...
			selector.find('.remove-file').unbind().addClass('upload-button').removeClass('remove-file').val(wix_l10n.upload);
